...
const Slideshow = ({ showSkeleton = false, gallery, state }: Props) => {
  return (
    <div className="relative slideshow shimmer scale-in xl:shadow-2xl pointer-events-none xl:aspect-[3/2] aspect-[3/1.5] rounded-md">
      ...
      <div className="absolute top-0 left-0 w-full h-full flex justify-between overflow-hidden rounded-md">
        ...
        {state.currentEntry && GalleryEntryType.isImage(state.currentEntry) && (
          <button
            ...
            className="interactable w-12 h-12 my-auto xl:drop-shadow-[0_0_1.0rem_rgba(0,0,0,1.0)] transition-opacity hover:opacity-85 opacity-15 pointer-events-auto"
          >
